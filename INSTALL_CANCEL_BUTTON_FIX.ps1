...
$backup = Join-Path $project "_ui_cancel_backup_$stamp"

New-Item -ItemType Directory -Force (Join-Path $backup "src\gui") | Out-Null
Copy-Item $target (Join-Path $backup "src\gui\main_window.py") -Force
...
Write-Host ""
Write-Host "FIX УСТАНОВЛЕН." -ForegroundColor Green
